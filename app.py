...
【金額が明記されている場合の特別ルール】★重要★
特約・覚書・重説などに具体的な金額が記載されている場合は必ずfixed_amountに数値を入れること：
- 例：「ハウスクリーニング費用として金55,000円（税別）を借主負担とする」→ fixed_amount: 55000
- 例：「エアコン洗浄費用 1台15,000円（税込）は借主負担」→ fixed_amount: 15000（税込の場合はtax_included: trueも付ける）
- 例：「鍵交換費用 実費とする」→ fixed_amount指定なし
- 金額が明記されていない場合はfixed_amountを省略（0にしない）
- 固定金額がある工事項目はbasisに「契約特約（金額固定）」と記載
...
{{
  ...
  "burden_assignments": {{
    ...
    "ハウスクリーニング（基本）": {{"burden": "借 or 貸", "basis": "契約特約 or 東京都ガイドライン", "reason": "理由", "fixed_amount": 55000, "tax_included": False}},
